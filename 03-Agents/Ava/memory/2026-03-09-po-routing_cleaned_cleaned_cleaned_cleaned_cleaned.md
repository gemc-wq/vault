...
**Decisions**
- **VCO Gap Analysis Scope (Approved by Cem)**: Covers entire process through fulfillment, including document review.
- **PO Routing Logic (Defined by Cem)**: Location-based (e.g., divert to PH if local printer is down) with fixed carrier rules.
- **Handoff Protocol**: Gap analysis briefs must be placed in the handoff folder with notification sent to Harry.
...
- **PO Workflow & Prioritization**: 
    - **Priority 1**: PH stock (PH morning) for label/print generation.
    - **Priority 2**: UK/US stock (afternoon waves) to meet 2 PM/3 PM local shipping cut-offs.

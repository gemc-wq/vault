...
### ⚠️ Inventory Model by Product Type
Some product types are print-on-demand from raw material rolls — they do NOT have per-device blank inventory.

| Inventory Model | Product Types | How It Works |
|----------------|---------------|-------------|
| **Blank-based** (device-specific inventory) | HTPCR, HB401, HB6CR, HB7BK, HC, HLBWH, HB1BK, HA805 | Pre-cut blanks per device. Stock tracked in `elcell_co_uk_barcode.t_sage_stock` / BQ `zero_dataset.inventory`. |
| **Roll-based** (no per-device inventory) | H8939, HSTWH, H9039, H7805 | Printed on vinyl rolls. Always "in stock" if roll material available. No blank inventory per device code. |
| **Sheet-based** (no per-device inventory) | HDMWH | Desk mats printed on sheet material. Size-based (900x400, 600x300), not device-based. |

**For coverage/inventory gap analysis:** Exclude H8939, HSTWH, H9039, H7805 from blank inventory cross-reference — their availability is material-based, not blank-based.
